...
export function resolveSidebarItems(page, regularPath, site, localePath) {
  ...
  if (!sidebarConfig) {
    ...
    if (typeof config === "string") {
      return resolveHeaders(page, config);
    }
 ...
 * @returns { SidebarGroup }
 */
function resolveHeaders(page, groupName) {
  const headers = groupHeaders(page.headers || []);
  return [
    {
      type: "group",
      collapsable: false,
      title: groupName ? groupName : page.title,
      path: null,
...
export function relatedBlogs({
  ...
  return $site.pages.filter(each =>
    each.path.match(new RegExp(`^${$localeConfig.path}blogs/[^/]+`))
  );
}
