...
sam local invoke --profile default --skip-pull-image OreviaService -e event.json -n env.json --docker-network $OPT_NETWORK --parameter-overrides ParameterKey=Environment,ParameterValue=local,ParameterKey=ProjectName,ParameterValue=orevia,ParameterKey=DDBEndpoint,ParameterValue=$OPT_DYNAMO_ENDPOINT,ParameterKey=ServiceOrigins,ParameterValue="http://localhost\,http://192.186.100.2",ParameterKey=OreviaDDBTableName,ParameterValue='orevia-orevia-local' $ADDITIONAL
